...
import numpy as np
import torch
from algorithms.logistic_regression import efficient_logistic_regression_function
from algorithms.sgd_classifier import sgd_random_search_v1
from algorithms.naive_bayes import naive_bayes_function
from algorithms.xgboost_model import xgboost_function
from algorithms.random_forest import random_forest_function
...
if torch.cuda.is_available():
    ...
    print("CUDA is not available. Ensure you have a CUDA-capable GPU and drivers installed.")
...
X_train,X_test, y_train, y_test = train_test_split(df, y_true.squeeze(), stratify=y_true, test_size=0.3)
...
_train = y_train.ravel() if isinstance(y_train, np.ndarray) else y_train.values.ravel()
y_test = y_test.ravel() if isinstance(y_test, np.ndarray) else y_test.values.ravel()
...
# Convert y_train_tensor and y_test_tensor to NumPy arrays
...
efficient_logistic_regression_function(X_train, X_test, y_train, y_test)
#
# naive_bayes_function(X_train, X_test, y_train, y_test)

# sgd_random_search_v1(X_train, X_test, y_train, y_test)
# random_forest_function(X_train, X_test, y_train, y_test)
# xgboost_function(X_train, X_test, y_train, y_test)
